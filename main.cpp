...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
	...
	int enemyHp = 100;        // ← 初期化
	int texturePath = 1;      // ← 初期化
	Log(std::format("enemyHp:{},texturePath:{}\n", enemyHp, texturePath));

	int wstringValue = 123;
	...
	RegisterClassW(&wc);       // ← RegisterClassW に変更
	...
	HWND hwnd = CreateWindowW(  // ← CreateWindowW に変更
		wc.lpszClassName,
		L"CG2",
		WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
	...
	if (hwnd == nullptr) return 0;  // ← null チェック追加
	...
	assert(SUCCEEDED(hr));
	hr = swapChain->GetBuffer(1, IID_PPV_ARGS(&swapChainResources[1]));  // ← 追加
	assert(SUCCEEDED(hr));
